...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("svm_classifier_resnet101.log", encoding='utf-8'),
...
class SVMHybridTrainer:
    ...
    def _load_data(self, train_dir, val_dir):
        ...
        # the order of feature extraction need to be consistent
    ...
    def _load_feature_extractor(self):
        # We load the best model trained by the ResNet101Trainer
        # and remove its final ‘Brain’ layer (The Classifier)
        logging.info("Loading the best ResNet101 model as the feature extractor...")

        # Find the best model checkpoint path
        best_model_path = os.path.join("resnet101_checkpoint", "best_resnet101_model.pth")
        if not os.path.exists(best_model_path):
            raise FileNotFoundError(f"Best model not found: {best_model_path}. Please run train_resnet101_model.py first.")
        
        checkpoint = torch.load(best_model_path, map_location=self.device)

        # Rebuild the model structure and load the saved weights
        model = resnet101() # Instantiate the ResNet101 architecture
    ...
    def _extract_features(self, data_loader):
        ...
        with torch.no_grad():
            for inputs, lbls in data_loader:
                ...
                # Get the features vector from our modified model
...
def main():
    ...
    try:
        ...
        logging.info("Please ensure you run train_resnet101_model.py first to create the 'best_resnet101_model.pth' file.")
