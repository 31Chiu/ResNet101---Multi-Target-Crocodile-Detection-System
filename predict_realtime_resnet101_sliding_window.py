...
def load_model(model_path):
    """
    Load and initialize the ResNet101 model from a checkpoint file.
    Args:
    ...
    # Initialize the ResNet101 model architecture without pretrained weights
...
def predict(model, image_tensor, class_names):
    ...
    Args:
        model: The loaded ResNet101 model.
...
def main():
    # Main function to run real-time object detection with ResNet101.
    # Path to the trained ResNet101 model checkpoint
    # This path is updated to match your training script's output.
    model_path = 'resnet101_checkpoint/best_resnet101_model.pth'

    if not os.path.exists(model_path):
        print(f'Error: Model file not found at {model_path}')
        print("Please ensure you have trained the ResNet101 model and the path is correct.")
        return
    ...
    while True:
        ...
        # After checking all windows, draw the box for the one with the highest confidence
        ...
        cv2.imshow("ResNet101 Sliding Window Detection", frame)
